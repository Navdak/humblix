...
<div class="form-grid">
    ...
    <div class="form-field full"><label>Challenge</label><textarea name="challenge" rows="3">{{ old('challenge',$project->challenge) }}</textarea></div>
    <div class="form-field full"><label>Solution</label><textarea name="solution" rows="3">{{ old('solution',$project->solution) }}</textarea></div>
    <div class="form-field full"><label>Result</label><textarea name="result" rows="3">{{ old('result',$project->result) }}</textarea></div>
    <div class="form-field full"><label>Outcome</label><textarea name="outcome" rows="3">{{ old('outcome',$project->outcome) }}</textarea></div>
    <div class="form-field full"><label>Equipment Used</label><input name="equipment_used" value="{{ old('equipment_used',$project->equipment_used) }}"></div>
    <div class="form-field full"><label>Safety Controls</label><textarea name="safety_controls" rows="3">{{ old('safety_controls',$project->safety_controls) }}</textarea></div>
