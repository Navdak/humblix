...
<div class="admin-card">
    ...
    @forelse($jobs as $job)
        <tr><td><strong>{{ $job->title }}</strong><small>{{ $job->closing_date ? 'Closes '.$job->closing_date->format('M j, Y') : 'No closing date' }}</small></td><td>{{ $job->department ?: '—' }}</td><td>{{ $job->location ?: '—' }}</td><td>{{ $job->employment_type ?: '—' }}</td><td><span class="status-badge status-{{ $job->status }}">{{ $job->status }}</span></td><td>{{ $job->published_at ? $job->published_at->format('M j, Y') : '—' }}</td><td class="admin-actions"><a class="btn btn-white" href="{{ route('admin.jobs.edit',$job) }}">Edit</a>@if(auth()->user()?->canDeleteRecords())<form method="POST" action="{{ route('admin.jobs.destroy',$job) }}" onsubmit="return confirm('Delete this job opening?')">@csrf @method('DELETE')<button class="btn btn-outline" style="color:#b91c1c">Delete</button></form>@endif</td></tr>
    @empty
