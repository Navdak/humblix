...
<div class="admin-card">
    ...
    @forelse($members as $member)<tr><td><strong>{{ $member->name }}</strong><br><span class="meta">{{ $member->experience }}</span></td><td>{{ $member->role }}</td><td>{{ $member->region }}</td><td>{{ $member->is_visible ? 'Yes' : 'No' }}</td><td class="admin-actions"><a class="btn btn-white" href="{{ route('admin.team.edit',$member) }}">Edit</a><form method="POST" action="{{ route('admin.team.destroy',$member) }}" onsubmit="return confirm('Delete this team member?')">@csrf @method('DELETE')<button class="btn btn-outline" style="color:#b91c1c">Delete</button></form></td></tr>@empty<tr><td colspan="5">No team members yet.</td></tr>@endforelse
    </tbody></table>
