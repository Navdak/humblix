...
<form class="admin-card" method="POST" action="{{ route('admin.settings.update') }}">
    ...
    @if(!empty($developerFields))
        ...
        <div class="form-grid" style="margin-top:16px">
            ...
            <div class="form-field"><label>Image Path</label><input type="text" name="technical_partner_image_path" value="{{ old('technical_partner_image_path', $settings['technical_partner_image_path'] ?? 'images/generated/careers/careers-office-admin-culture.jpg') }}"><small>Use a public image path for now. Example: images/generated/careers/careers-office-admin-culture.jpg</small></div>
            <div class="form-field"><label>Portfolio URL</label><input type="url" name="technical_partner_portfolio_url" value="{{ old('technical_partner_portfolio_url', $settings['technical_partner_portfolio_url'] ?? '') }}" placeholder="https://example.com/portfolio"><small>Public card uses a dummy portfolio link until you save your real one.</small></div>
